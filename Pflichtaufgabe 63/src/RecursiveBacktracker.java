...
public class RecursiveBacktracker implements MazeGenerator {
    ...
    public char[][] generate(int height, int width){
        if (height < 5 || width < 5) {
            throw new IllegalArgumentException("Das Feld muss mindestens 5x5 gross sein.");
        }
    ...
    /**
     *  Entfernt die Wand zu einem unbesuchten Nachbarn und ruft sich selbst mit dem Nachbarn nochmal auf
     *  @param map Karte
     *  @param besucht besuchte Felder
     *  @param positionx aktuelle x Position
     *  @param positiony aktuelle y Position
     *  @return map
     */
    public char[][] recursive(char[][] map, boolean[][] besucht, int positionx, int positiony){
        ...
        while (frei[4] > 0) {
            ...
            int nachbar = (frei[4] > 0) ? frei[(int) (Math.random() * frei[4])] : 0;
            switch (nachbar) {
                case 1:
